...
class DeformableTransformer(nn.Module):
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        ...
        # 根据层级调整proposal尺寸分布
        if lvl <= 1:  # 低分辨率层，生成更多大尺寸proposals
            base_wh = 0.05 * (2.0 ** lvl)
            # 添加多种尺寸的proposals
            wh_variants = [base_wh, base_wh * 0.5]  # 小、大
            for wh_scale in wh_variants:
                wh = torch.ones_like(grid) * wh_scale
                proposal = torch.cat((grid, wh), -1).view(N_, -1, 4)
                proposals.append(proposal)
        elif lvl >= 4:  # 高分辨率层，生成更多小尺寸proposals
            wh_variants = [base_wh * 2, base_wh]
            for wh_scale in wh_variants:
                wh = torch.ones_like(grid) * wh_scale
                proposal = torch.cat((grid, wh), -1).view(N_, -1, 4)
                proposals.append(proposal)
